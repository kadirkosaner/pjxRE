...
base = Path('1Developer/Wardrobe/Prompts/14 - BikiniTop (Done)')
...
for f in files:
    ...
    pos = 0
    while True:
        start = text.find('"id": "bikini_top_', pos)
        if start == -1:
            break
        ...
        try:
            ...
            if isinstance(obj, dict) and obj.get('slot') == 'bikiniTop' and obj.get('id', '').startswith('bikini_top_'):
                ...
                for v in obj.values():
                    if isinstance(v, dict) and v.get('slot') == 'bikiniTop' and v.get('id', '').startswith('bikini_top_'):
...
print('Total bikini tops:', len(all_items))
...
Path('bikini_top_db_lines.txt').write_text('\n'.join(out), encoding='utf-8')
for store, ids in store_ids.items():
    Path(f'bikini_top_ids_{store}.txt').write_text('\n'.join(ids), encoding='utf-8')
print('Wrote bikini_top_db_lines.txt and bikini_top_ids_*.txt')
