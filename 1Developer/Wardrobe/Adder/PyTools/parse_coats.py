...
base = Path('1Developer/Wardrobe/Prompts/04 - Coats (Done)')
...
for f in files:
    ...
    while True:
        ...
        for i in range(obj_start, min(obj_start + 1500, len(text))):
            if text[i] == '{':
                depth += 1
            elif text[i] == '}':
                depth -= 1
        ...
        try:
            ...
            if isinstance(obj, dict) and obj.get('slot') == 'coat' and obj.get('id', '').startswith('coat_'):
                ...
                # outer wrapper like {"coatNorthX": {...}}
                for v in obj.values():
                    if isinstance(v, dict) and v.get('slot') == 'coat' and v.get('id', '').startswith('coat_'):
                        v.pop('reqHeelsSkill', None)
                        oid = v['id']
                        if oid in seen_ids:
                            suffix = {'storeClothingB': '_vera', 'storeClothingC': '_fifth', 'storeSports': '_sports'}.get(v.get('store'), '_alt')
                            v['id'] = oid + suffix
                        seen_ids.add(v['id'])
                        store = v.get('store', '')
                        if store in store_ids:
...
Path('coats_db_lines.txt').write_text('\n'.join(out), encoding='utf-8')

for store, ids in store_ids.items():
    Path(f'coats_ids_{store}.txt').write_text('\n'.join(ids), encoding='utf-8')

print('Wrote coats_db_lines.txt and coats_ids_*.txt')
