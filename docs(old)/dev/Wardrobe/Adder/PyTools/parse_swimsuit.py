...
base = Path('1Developer/Wardrobe/Prompts/16 - Swimsuit (Done)')
...
for f in files:
    ...
    pos = 0
    while True:
        start = text.find('"id": "swimsuit_', pos)
        if start == -1:
            break
        ...
        try:
            ...
            if isinstance(obj, dict) and obj.get('slot') == 'swimsuit' and obj.get('id', '').startswith('swimsuit_'):
                ...
                for v in obj.values():
                    if isinstance(v, dict) and v.get('slot') == 'swimsuit' and v.get('id', '').startswith('swimsuit_'):
...
print('Total swimsuits:', len(all_items))
...
Path('swimsuit_db_lines.txt').write_text('\n'.join(out), encoding='utf-8')
for store, ids in store_ids.items():
    Path(f'swimsuit_ids_{store}.txt').write_text('\n'.join(ids), encoding='utf-8')
print('Wrote swimsuit_db_lines.txt and swimsuit_ids_*.txt')
